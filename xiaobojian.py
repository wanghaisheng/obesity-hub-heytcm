...
def segment_chinese_title_jieba(title):
    ...
    if '治疗' in title:
        title=title.split('治疗')[0]
    if '对' in  title:
        title=title.split('对')[0]
    if '通过' in title:
        title=title.split('通过')[0]
    if '研究' in title:
        title=title.split('研究')[0]
    if '干预' in title:
        title=title.split('干预')[0]
    if '改善' in title:
        title=title.split('改善')[0]
    if '调控' in title:
        title=title.split('调控')[0]
    if '防治' in title:
        title=title.split('防治')[0]
    if '预防' in title:
        title=title.split('预防')[0]
    seg_list = jieba.lcut(title, cut_all=True)
    # seg_list=[title]
    return seg_list

# --- Main script ---
input_filename = "小檗碱-学位论文.tsv"
data_from_file = ""
...
custom_terms = [
    ...
    "“治未病”", "“病证结合”", "“四联全生态”", "“互联网+”", "“肝肾同源”理论","小檗碱",'黄连素'
]
    ...
    if title_val not in seen_titles:
        ...
        for word in segmented_title_list:
            ...
            if len(word_cleaned) > 1 or word_cleaned.isalnum(): # Keep words longer than 1 char, or single alphanumeric chars
                if word_cleaned not in ['及其','的', '与', '及', '和', '对', '中', '基于', '分析', '研究', '可视化', '趋势', '现状', '热点', '及', '（', '）', '(', ')', '近', '年', '我国', '国内', '相关', '进展', '应用', '探索', '探讨', '比较', '领域', '展望', '回顾', '前沿', '动态', '作用', '机制', '效果', '评价', '构建', '体系', '模式', '规律']: # Example stop words
...
output_filename_md = "processed_articles_jieba_output-小檗碱.md"
try:
...
word_freq_filename = "word_frequencies-小檗碱.txt"
try:
